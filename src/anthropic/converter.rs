...
/// Kiro CLI 默认 agent 指令
const CLI_DEFAULT_AGENT_PROMPT: &str = "# Kiro CLI Default Agent
...
## Key Capabilities
- Use the `use_subagent` tool when a task can be split into independent subtasks.
- Prefer code intelligence and repository-aware tools before broader searches.
- Work directly in the user's current workspace and use available tools to inspect, edit, and validate changes.";

/// Kiro CLI 默认 agent 的确认回复
const CLI_DEFAULT_AGENT_ACK: &str = "I will fully incorporate this information when generating my responses, and explicitly acknowledge relevant parts of the summary when answering questions.";
...
/// - 其他非空模型 ID（如 glm-5）按原样透传，避免拦截上游动态模型
...
fn cli_builtin_tool(name: &str, description: &str, schema: serde_json::Value) -> Tool {
    Tool {
        tool_specification: ToolSpecification {
            name: name.to_string(),
            description: description.to_string(),
            input_schema: InputSchema::from_json(normalize_json_schema(schema)),
        },
    }
}

fn cli_builtin_tools() -> Vec<Tool> {
    vec![
        cli_builtin_tool(
            "code",
            "Code intelligence for symbol lookup, AST search, and repository-aware navigation.",
            serde_json::json!({
                "type": "object",
                "properties": {
        ...
        cli_builtin_tool(
            "dummy",
            "Placeholder tool used when a previously available tool no longer exists.",
            serde_json::json!({
                "type": "object",
                "properties": {},
                "required": []
        ...
        cli_builtin_tool(
            "fs_read",
            "Read files, directories, search results, or images from the workspace.",
            serde_json::json!({
                "type": "object",
                "properties": {
                    "operations": {
        ...
        cli_builtin_tool(
            "grep",
            "Search for regex patterns in repository files.",
            serde_json::json!({
                "type": "object",
                "properties": {
                    "pattern": {"type": "string"},
        ...
        cli_builtin_tool(
            "introspect",
            "Inspect this chat application's documented features and settings.",
            serde_json::json!({
                "type": "object",
                "properties": {
                    "query": {"type": "string"},
        ...
        cli_builtin_tool(
            "report_issue",
            "Open a prefilled GitHub issue for chat bugs or feature requests.",
            serde_json::json!({
                "type": "object",
                "properties": {
                    "title": {"type": "string"},
        ...
        cli_builtin_tool(
            "session",
            "Temporarily inspect or update session settings.",
            serde_json::json!({
                "type": "object",
                "properties": {
                    "operation": {"type": "string"},
        ...
        cli_builtin_tool(
            "use_aws",
            "Run an AWS CLI style operation.",
            serde_json::json!({
                "type": "object",
                "properties": {
                ...
                "required": ["service_name", "region", "operation_name", "label"]
            }),
        ),
        cli_builtin_tool(
            "use_subagent",
...
fn extend_with_cli_builtin_tools(tools: &mut Vec<Tool>) {
    let mut existing_names: std::collections::HashSet<String> = tools
        .iter()
        .map(|tool| tool.tool_specification.name.to_lowercase())
        .collect();

    for tool in cli_builtin_tools() {
        let tool_name = tool.tool_specification.name.to_lowercase();
        if existing_names.insert(tool_name) {
            tools.push(tool);
        }
    }
}
...
pub fn convert_request(req: &MessagesRequest) -> Result<ConversionResult, ConversionError> {
    ...
    if use_cli_agent_envelope {
        ...
        extend_with_cli_builtin_tools(&mut tools);
    }
    ...
    let content = if use_cli_agent_envelope {
        wrap_cli_current_message_content(&text_content)
    } else {
        text_content
    };
...
fn should_apply_cli_agent_envelope(model_id: &str) -> bool {
...
fn load_cli_workspace_context_entry() -> Option<String> {
    ...
    for candidate in ["README.md", "README.MD", "readme.md"] {
        ...
        let content = truncate_chars(&content, 12_000);
        return Some(format!(
            "--- CONTEXT ENTRY BEGIN ---\n[{}]\n{}\n--- CONTEXT ENTRY END ---",
            path.display(),
            content.trim_end()
...
fn build_cli_default_agent_instruction() -> String {
    match load_cli_workspace_context_entry() {
        Some(workspace_context) => {
            format!(
                "{}\n\nFollow this instruction: {}",
                workspace_context, CLI_DEFAULT_AGENT_PROMPT
            )
        }
        None => format!("Follow this instruction: {}", CLI_DEFAULT_AGENT_PROMPT),
    }
}

fn prepend_cli_default_agent_history(history: &mut Vec<Message>, model_id: &str) {
    let default_agent_instruction = build_cli_default_agent_instruction();
    history.insert(
        0,
        Message::Assistant(HistoryAssistantMessage::new(CLI_DEFAULT_AGENT_ACK)),
    );
    history.insert(
        0,
        Message::User(HistoryUserMessage::new(default_agent_instruction, model_id)),
    );
}

fn wrap_cli_current_message_content(content: &str) -> String {
...
mod tests {
    ...
    #[test]
    fn test_map_model_empty_is_unsupported() {
        assert!(map_model("   ").is_none());
    }

    #[test]
    fn test_cli_agent_envelope_enabled_for_passthrough_model() {
        ...
        if let Some(Message::User(user)) = history.first() {
            assert!(
                user.user_input_message
                    .content
                    .contains("Kiro CLI Default Agent")
            );
        }
        assert!(
            result
        ...
        assert!(
            result
                ...
                .tools
                .iter()
                .any(|tool| tool.tool_specification.name == "use_subagent")
        );
    }

    #[test]
    fn test_cli_agent_envelope_disabled_for_claude_model() {
        use super::super::types::Message as AnthropicMessage;
